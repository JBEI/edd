...
####################################################################################################
# Column header patterns for the experiment definition file.
...
class ColumnLayout:
    ...
    class _ExperimentDefNamingStrategy(NamingStrategy):
        """
        A simple line/assay naming strategy assumed in the experiment definition file use case,
        where line names are
        ...
        def get_line_name(self, strains, line_metadata, replicate_num, line_metadata_types,
                          is_control):
            replicate_suffix = (
            ('-R%d' % replicate_num) if self.combinatorial_input.replicate_count > 1
            else '')
        ...
        def get_assay_name(self, line, protocol, assay_metadata, assay_metadata_types):
            try:
                ...
                raise ValueError(
                    KeyError)  # raise more generic Exception published in the docstring

    class _InputFileRow(CombinatorialDefinitionInput):
        """
        A special case of combinatorial line/assay creations in support of experiment definition
        file
    ...
    class ExperimentDefFileParser(CombinatorialInputParser):
        ...
        def __init__(self, protocols_by_pk, line_metadata_types_by_pk, assay_metadata_types_by_pk,
            ...
            if len(self.assay_metadata_types_by_name) != len(assay_metadata_types_by_pk):
                logger.warning(
                        'Found some assay metadata types that differ only by case. '
                        'Case-insensitive '
                        'matching in this function will arbitrarily choose one')
        ...
        def read_column_layout(self, row):
            ...
            minimal required column headers, then comparing additional headers in that row
            against line and
            assay metadata names in EDD's database. If required column headers aren't found in
            this row,
            ...
            for col_index in range(len(row)):
                ...
                # check whether the column label matches custom data defined in the database
                    ...
                    if line_metadata_type is None:
                        result_dest = self.errors if self.REQUIRE_COL_HEADER_MATCH else \
                            self.warnings
        ...
        def _parse_assay_metadata_header(self, layout, upper_content, col_index):
            ...
            MetadataType
            if one was found, or True if it was clearly intended to be one, but was logged as an
            error).
            """
            ...
            for upper_protocol_name, protocol in self.protocols_by_name.items():
                if upper_content.startswith(upper_protocol_name):
                    ...
                    if suffix_meta_type:
                        layout.register_assay_meta_column(col_index, upper_protocol_name, protocol,
                                                          suffix_meta_type, is_combinatorial)
        ...
        def read_row(self, cols_list, row_num):
            ...
            if layout.line_control_col is not None:
                ...
                cell_content = self._get_string_cell_content(cols_list, row_num,
                                                             layout.line_control_col, errors)
            ...
            if layout.replicate_count_col is not None:
                ...
                try:
                    ...
                    self._append_to_errors_list(invalid_count, cell_content)

            ###################################################
            # Strain part id(s)
            ###################################################
            # TODO: after some initial testing, consider adding custom group support for Carbon
            # Source
            ...
            # TODO: after using the combinatorial strain creation code here for some testing of
            # other
            # parts of the back end, consider removing it since it will result in creation of
            # lines with
            # duplicate names
            if layout.strain_ids_col is not None:
                ...
                if tokens:
                    ...
                    for token in tokens:
                        ...
                        # included
                        # in the list. That means that each top-level comma-delimited entry in
                        # the list
                        ...
                        if strain_group_match:
                            strain_group = (strain_id.strip() for strain_id in
                                            strain_group_match.group(1).split(';'))
                        ...
                        # we'll allow all input through to the ICE query later in case our
                        # pattern is
                        # dated, but this way we can provide a more helpful prompt for bad user
                        # input
                        ...
                        if not part_number_match:
                            ...
                            if not unmatched_part_nums:
                                warnings[
                                    PART_NUMBER_PATTERN_UNMATCHED_WARNING] = unmatched_part_nums
            ...
            if layout.col_index_to_line_meta_pk:
                for col_index, line_metadata_pk in layout.col_index_to_line_meta_pk.items():
                    cell_content = self._get_string_cell_content(cols_list, row_num, col_index,
                                                                 errors)
            ...
            if layout.col_index_to_assay_data:
                ...
                for col_index, (
                protocol, assay_metadata_type) in layout.col_index_to_assay_data.items():
                    ...
                    if col_index in layout.combinatorial_col_indices:
                        ...
                        self._parse_combinatorial_input(row_inputs, cell_content, row_num,
                                                        col_index, assay_metadata_type.pk,
                                                        error_key, protocol, parser)

                    else:
                        if row_inputs.has_assay_metadata_type(protocol.pk, assay_metadata_type.pk):
                            # TODO could improve this error content with a more complex data
                            # structure
                            self._append_to_nested_errors_list(errors,
                                    'duplicate_assay_matadata_columns', protocol.pk,
        ...
        def _parse_combinatorial_input(self, row_inputs, cell_content, row_num, col_index,
            ...
            for token in cell_content.split(','):
                ...
                try:
                    ...
                    logger.warning(
                        'ValueError parsing token "%(token)s" from cell content "%(cell)s" '
                        'in row_num=%(row)d col_num='
                        '%(col)d' % {
                            'token': token, 'cell': cell_content, 'row': row_num,
                            'col': col_index + 1,
                        })
...
class _InputFileRow(CombinatorialDefinitionInput):
    """
    A special case of combinatorial line/assay creations in support of experiment definition file
    upload. Each line of the file is itself a combinatorial line/assay creation, at
    least if protocols/times are included, or the more advanced combinatorial features used.
    One-line-per-row creation, which is what users will likely use templates for most
    often, is just a degenerate case of combinatorial creation.
