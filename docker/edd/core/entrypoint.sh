...
if [ -z "${EDD_USER}" ]; then
    (>&2 echo "No EDD_USER environment set; did you run init-config before launching Docker?")
    exit 1
elif [ -z "${EDD_EMAIL}" ]; then
    (>&2 echo "No EDD_EMAIL environment set; did you run init-config before launching Docker?")
    exit 1
fi

output "EDD_DEPLOYMENT_ENVIRONMENT: ${EDD_DEPLOYMENT_ENVIRONMENT:-'Not specified'}"
...
EDD_VERSION_HASH="$(cat /edd.hash)"
export EDD_VERSION_HASH

# If specified, wait on other service(s) -- including main edd service
...
# render error page if needed -- depends on staticfiles being initialized
if [ ! -f "/code/edd/main/templates/500.html" ]; then
    output "Rendering 500.html error page"
    python manage.py edd_render_error
fi
...
# make sure directories are owned by edduser before launching final command
ensure_dir_owner "/usr/local/edd" "edduser"
ensure_dir_owner "/var/log/edd" "edduser"
