...
class DataGridServerSide {
	...
	clickedSort(thisth) {
		...
		setTimeout( () => this._sortIt(thisth), 1 );
	...
	setColumnVisibility(columnID, visible):void {
		...
		// Apply the style to the filtered list of headers.
	...
	getColumnCells(columnID, includeHeaders):any {
	...
	private _handleRowToggleTable() {
		...
		var tableWithTogglesID = <any>this._table.getAttribute("rowToggleTable");
		if (tableWithTogglesID) {
			var tableWithToggles = <any>document.getElementById(tableWithTogglesID);
			if (tableWithToggles) {
		...
		if (triggerButtonID) {
			...
			if (this._triggerButton) {
				...
				this._triggerButton.onclick = function() { pThis._clickedRefresh(); };
			}
		}
	}
	...
	private _clickedRefresh() {
		...
		setTimeout( function() { pThis._refreshIt(); }, 1, this );
	}
	...
	private _refreshIt() {
		...
		for(var i:number = 0; i < tabledatarows.length; i++) {
			rowdisplayflags[i] = 0;
		}
	...
	private _handleSelectAllButton(oneToggleControl):void {
		...
		oneToggleControl.addEventListener('click', () => DataGridServerSide._clickedRowControl(oneToggleControl), false);
	}
	...
	private _isRowEnabled(rowIndex):any {
	...
	private _handleShowRowsButton(oneToggleControl):void {
		this._showRowsButton = oneToggleControl;
		oneToggleControl.dataGridToRefresh = this;
		$(oneToggleControl).click( () => DataGridServerSide._clickedRowControl(oneToggleControl) );
		...
		if (showvaluesattrib) {
			...
			for(var i:number = 0; i < this._datarows.length; i++) {
			...
			// for later use.
			var showvalues = showvaluesattrib.split(',');			
			for (var splitindex:number = 0; splitindex < showvalues.length; splitindex++) {
				var sindex = parseInt(showvalues[splitindex], 10);
	...
	private _handleMultiTableShowRowsButton(oneToggleControl):void {
	...
	private _handleShowRowsPulldown(oneToggleControl):void {
		var pThis = this;

		oneToggleControl.onclick = function() { 
			setTimeout( function() { pThis._refreshRowObjects(); }, 1 ); 
		};
	...
	private _refreshRowObjects():void {
		...
		for (var controlindex = 0; controlindex < controlsArray.length; controlindex++) {
			...
			if (controlType == 'showRowsButton') {
				if (oneControl.enabledState == 1) {
					// Apply the mask we just built to the master flag list
			...
			if (controlType == 'showRowsPulldown') {
				...
				if (oneop.showarray) {
					...
					if (optionValue == "all") {
					// If there is no array defined and we want to show "all", do nothing
					} else {
					// Otherwise, turn off every single row (the array probably doesn't exist
					// because the original string was empty)
						for (i = 0; i < tabledatarows.length; i++) {
							rowdisplayflags[i] = 0;
						}							
					}
		...
		// Now we run through the set of controls again, handling all the controls that
		// rely on a proper set of display flags, as constructed by the earlier controls.
				...
				if (oneControl.enabledState == 1) {
					...
					for (i = 0; i < tabledatarows.length; i++) {
						...
						for (var c = 0; c < tabledatarows[i].cells.length; ++c) {
							...
							if (!cb) { continue; }
							if (!cb[0]) { continue; }
							if (cb[0].type.toLowerCase() != 'checkbox') { continue; }
	...
	private _stripeIt():void {
		...
		for (var j = 0; j < rowstocheck.length; j++) {	
			...
			if (row.applyStriping == true) {
				...
				if (row.compRow == true) {
					// Do not swap striping if the row is a 'companion' to the one above
				} else {
					striping = 1 - striping;
				}
	...
	private _handlePulldownOrColumnMenu(oneToggleControl):void {
		oneToggleControl.dataGridToRefresh = this;
		...
		if (labelOff && labelOn && menuBlock) {
			...
			$(labelOff).click( () => DataGridServerSide._clickedColMenuWhileOff(labelOff) );
			$(labelOn).click( () => DataGridServerSide._clickedColMenuWhileOn(labelOn) );
		}
	}


	private _handleColumnMenu(oneToggleControl):void {
		...
		for (var c = 0; c < cb.length; ++c) {
			if (cb[c].type.toLowerCase() == 'checkbox') {
				$(cb[c]).click( () => DataGridServerSide._clickedColControl(oneToggleControl) );
	...
	private _getColumnIndexByID(columnID):any {
		var headerCells = this._getHeaderCells();
	...
	private _getHeaderRows(headerrows, nonheaderrows):void {
		// For each row of the main table,
	...
	private _handleSortable():void {
		...
		// Run through and check some attributes
		for (var j = 0; j < rowstocheck.length; j++) {
		...
		for (var j = 0; j < headerrows.length; j++) {
			...
			// If it has any header cells in it,
			if (foundcells.length) {	
				// Add a click event for every header cell that contains our class
				for (var k = 0; k < foundcells.length; k++) {
					var thisheadcell = foundcells[k];
					if (thisheadcell.tagName.toLowerCase() == 'th') {		
						if ($(thisheadcell).hasClass('sortheader')) {
							...
							var func = function( headCell ) { return function() { pThis.clickedSort( headCell ); } }
	...
	private _handleColumnCheckbox( oneToggleControl ):void {
		oneToggleControl.dataGridToRefresh = this;
		if (oneToggleControl.type) {
			if (oneToggleControl.type.toLowerCase() == 'checkbox') {
				$(oneToggleControl).click( () => DataGridServerSide._clickedColControl(oneToggleControl) );
	...
	private _getLineIDFromRow(row):any {
		return row.getAttribute('data-line-id');
	}


	// Find a row from its line ID.
	private _getRowByLineID(lineID):any {
	...
	private _refreshColObjects():void {
		...
		if ((this._datarows) && (this._tablecolcontrols)) {
			...
			// Count the cells in the first data row, and set a bitfield based on the count
	...
	private _sortIt(thisth, sameSortOrder:boolean = false):void {
	...
	private _getIndexedList(inputList, order):any {
	...
	private _reinsertRows(rows):void {
	...
	static scanPageForTables():void {
	    ...
	    for (var i=0; i<ts.length; i++) {
			...
			(<any>table).DataGridServerSide = new DataGridServerSide(table, tablebody);
		}
	}


	// add/remove 'on' and 'off' CSS styles to reflect visibility.
	private static _setRowVisible(row, isVisible):void {
		if (isVisible) {
			$(row).addClass('on');
			$(row).removeClass('off');
		} else {
	...
	private static _isRowVisible(row):any {
		return !$(row).hasClass('off');
	}


	// 'control' is a button.
	private static _clickedRowControl(control:any):any {
		...
		if (dataGridRef) {
			// We turn the rest of the operation into an event so the browser
			// will (probably) refresh, showing our 'please wait' message
			setTimeout( function() { dataGridRef._refreshRowObjects(); }, 1 );
	...
	private static _clickedColMenuWhileOff(control:any):void {
	...
	private static _clickedColMenuWhileOn(control:any):void {
		if (control.menuBlock) {
	...
	private static _clickedColControl(control:any):void {

		var dataGridRef = control.dataGridToRefresh;
		if (dataGridRef) {
			// We turn the rest of the operation into an event so the browser
			// will (probably) refresh, showing our 'please wait' message
			setTimeout( function() { dataGridRef._refreshColObjects(); }, 1 );
		}
	}
	...
	private static _parseNumberList( inList, outList ):void {
	...
	private static _updateColumnSettings(obj):any {
	...
	private _tableMainElementID:string;
	
	private _headerrows:any;
	private _datarows:any;

	private _tablerowcontrols:any;
	private _tablecolcontrols:any;

	private _lastHeaderThatSorted:any = null;
	private _showRowsButton:any = null;			// This is the button with tableControlType=showRowsButton
	...
	private _enabledRows:any = null;

	private _groupIDToRowIDs:any;
	private _groupIDToRows:any;
	private _groupIDToHeader:any;

	private _groupsCollapsed:any;

	private _helper:DataGridHelperBase;

	private _triggerButton:any;
	private _checkboxestoprocess:any;

	private _stripingA:string;
	private _stripingB:string;
}
...
window.addEventListener('load', function() { DataGridServerSide.scanPageForTables(); }, false);
