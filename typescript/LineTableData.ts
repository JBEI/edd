...
processImportSettings:function() {
	...
	var mainRadioL = <HTMLInputElement>document.getElementById("rbLayoutMetaLines");
	var mainRadioC = <HTMLInputElement>document.getElementById("rbLayoutIonCSV");
	var dModeOptionsDiv = <HTMLInputElement>document.getElementById("dmodeoptions");
	...
	if (mainRadioC.checked) {
		...
		var rawdataformatp = <any>document.getElementById("rawdataformatp");
		if (rawdataformatp) {
	...
	var p = <any>document.getElementById("dlayoutp");
	if (p == null) {
		return;
	}
	...
	// We need an inverted version of the index-to-ID array for Lines.
	for (var o:any=0; o < EDDData.EnabledLineIDs.length; o++) {
		i = EDDData.EnabledLineIDs[o];
		lineIndexes[i] = o;
	}

	// In the completed table, column 0 is always line name, and is not part of tempGraphData.
	// The tempGraphData grid is grafted into the completed table from column 1 onward.
	//
	// Within tempGraphData:
	//   Column 0 is always Strain
	//   Column 1 is always Media
	//   Column 2 is a single Carbon Source.
	//      (Since there can be multiple Carbon Sources per Line, this represents a possible truncation of the existing data.
	//		 The only reason this is considered acceptable is that most of the time, a user will choose to 'marge' the data
	//		 on this page with pre-existing data, in which case the single Carbon Source will be consolidated into the set
	//		 with no data loss.
	//			Note that it is still possible to import more than one Carbon Source per Line:
	//       A user wishing to import more than one per Line need only specify the Carbon Source label for several columns.)
	//   Column 3 is Experimenter (as a full name)
	//   Column 4 is Contact (as an email address string, possibly resolved to a user)
	//   Columns 5+ are metadata
	//
	// This is reflected in the generalLineDataTypes variable as well.

	for (var x:any=0; x < EDDData.EnabledLineIDs.length; x++) {
		i = EDDData.EnabledLineIDs[x];
		var x:any =	lineIndexes[i];
	...
	var j = 5;
	for (var x:any=0; x < EDDData.MetaDataTypesRelevant.length; x++) {
	...
	// Here we store the current metadata associations
	// in the metaDataPulldownSettings hashes, labeling them by the order their data will be
	// parsed from the Step 2 text box, so we can accurately reproduce those settings
	// in Step 4.
	
	// Note that we will not do this for Lines, since we are going to depend on the Line name
	// being a unique identifier to do a reverse-lookup in Step 4.  Why?
	// Because a user pasting in a table of metadata wants pasted rows to match by name, not by index,
	// and if the names do not match, it's far more likely that a user is pasting in data for brand new Lines,
	// rather than badly-labeled or badly-ordered data for pre-existing Lines.
	...
	var generalAndMeta = [];	
	for (var o:any=0; o < EDDLTD.generalLineDataTypes.length; o++) {
	...
	if (generalAndMeta.length > 0) {
		if (EDDLTD.metaDataAre == "colsAreMeta") {
			for (var y:any=0; y < generalAndMeta.length; y++) {
			...
			for (var x:any=0; x < EDDData.EnabledLineIDs.length; x++) {
				...
				if (x < (EDDData.EnabledLineIDs.length - 1)) { // No final CR
					textBoxContent = textBoxContent + "\n";
				}
			}
		} else {
			for (var y:any=0; y < EDDData.EnabledLineIDs.length; y++) {
				lid = EDDData.EnabledLineIDs[y];
				textBoxContent = textBoxContent + "\t" + EDDData.Lines[lid].n.replace(/\t\t*/, '');
			}
			textBoxContent = textBoxContent + "\n";
			for (var x:any=0; x < generalAndMeta.length; x++) {
				...
				if (x < (generalAndMeta.length - 1)) { // No final CR
					textBoxContent = textBoxContent + "\n";
				}
			}		
		}
...
parseAndDisplayText:function() {
	...
	if (EDDLTD.interpretationMode == "ionCSV") {
		rows.push("Strain/Line Label" + delimiter + "Strain" + delimiter + "Line Label" + delimiter + "Induction" + delimiter +
					"Run Date" + delimiter + "Protocol" + delimiter + "Metabolite Type" + delimiter + "Measurement Time");
	}

	// Filter out rows that have no values in them
    for (var y in unfilteredrows) {
    	if (unfilteredrows[y] != "") {
			var cells = unfilteredrows[y].split(delimiter);
			if (cells.length > 0) {
		...
		if (parseInt(y) < 1) {
			...
			if (!multiColumn) {
				...
				if (EDDLTD.interpretationMode == "ionCSV") {
					...
					// Make sure we drop anything that's undefined
					...
					// Append the remaining cells as potential ratio values
					for (var x in cells) {
	...
	if (EDDLTD.graphXMarkers.length < longestRow) {
	    ...
	    for (var r:any = initialLength; r < longestRow; r++ ) {
	...
	// Fill in any open spots in the structure
	for (var i:any=0; i < longestRow; i++) {
	...
	// An important thing to note here is that this data is in [x][y] format -
	// that is, it goes by row, then by column, when referencing.
	// By contrast, the tableDataObjects array of arrays goes by column,
	// then by row - [c][r] ([y][x]).

	for (var x:any=0; x < longestRow; x++) {
		if (typeof EDDLTD.activeFlagsData[x] == 'undefined') {
			EDDLTD.activeFlagsData[x] = [];
		}
		for (var y:any=0; y < EDDLTD.graphYMarkers.length; y++) {
			if (typeof EDDLTD.activeFlagsData[x][y] == 'undefined') {
				EDDLTD.activeFlagsData[x][y] = 1;
			}
		}
	}
	for (var x:any=0; x < EDDLTD.graphXMarkers.length; x++) {
	...
	var aTD = <any>document.createElement("td");
	aTD.className = 'yMarkerCell';
	aTD.setAttribute('x',1);
	aTD.setAttribute('y',1);
	EDDLTD.tableYHeaderObject = aTD;

	// The header cells for the data columns
	for (var i:any=0; i < EDDLTD.graphXMarkers.length; i++) {
		aTD = <any>document.createElement("td");
	...
	// The checkboxes that go next to the header cells for the data columns
	for (var i:any=0; i < EDDLTD.graphXMarkers.length; i++) {
		aTD = <any>document.createElement("td");
		aTD.setAttribute('x',2+i);
	...
	// Header cells for the row labels, running down the left side of the table
	for (var i:any=0; i < EDDLTD.graphYMarkers.length; i++) {
		aTD = <any>document.createElement("td");
		aTD.className = 'yMarkerCell';
	...
	// The checkboxes that go next to the row label cells
	for (var i:any=0; i < EDDLTD.graphYMarkers.length; i++) {
		aTD = <any>document.createElement("td");
	...
	for (var i:any=0; i < EDDLTD.graphData.length; i++) {
		EDDLTD.tableDataObjects[i] = [];
		for (var j:any=0; j < EDDLTD.graphData[i].length; j++) {
			aTD = <any>document.createElement("td");
	...
	var tableObject = <any>document.createElement("table");
	tableObject.setAttribute('cellspacing', "0");
	var tBodyObject = <any>document.createElement("tbody");
	...
	var colGroupObject = <any>document.createElement("colgroup");
	...
	var aCol = <any>document.createElement("col");
	...
	// Then add columns for all the lines
	for (var i:any=0; i < EDDLTD.graphXMarkers.length; i++) {
		var aCol = <any>document.createElement("col");
	...
	var aRow = <any>document.createElement("tr");
	...
	aRow = <any>document.createElement("tr");
	...
	// The rest of the rows: A checkbox, a Y-axis marker, and column data.
	for (var i:any=0; i < EDDLTD.graphYMarkers.length; i++) {
		...
		for (var j:any=0; j < EDDLTD.tableDataObjects[i].length; j++) {
			aRow.appendChild(EDDLTD.tableDataObjects[i][j]);
		}
	}

	var dataTableDiv = <any>document.getElementById("dataTableDiv");
...
resetEnabledFlagMarkers:function() {
	...
	for (var i=0; i < EDDLTD.graphXMarkers.length; i++) {
		var aCB = <any>document.getElementById("enableColumn" + i);
		if (aCB != null) {
			aCB.checked = true;
		}
	}
	...
	for (var i=0; i < EDDLTD.graphYMarkers.length; i++) {
		var aCB = <any>document.getElementById("enableRow" + i);
		if (aCB != null) {
			aCB.checked = true;
		}
	}
...
remakeInfoTable:function() {
	...
	for (var key in EDDLTD.metaDataSegmentLabels) {
		...
		for (var o=0; o < EDDLTD.generalLineDataTypes.length; o++) {
			var v:any = EDDLTD.generalLineDataTypeLabels[o];
			aOPT = document.createElement("option");
		...
		var aGRP = <any>document.createElement("optgroup");
		if (EDDLTD.interpretationMode == "metaData") {
...
remakeDisambiguationTable:function() {
	...
	for (var tci=0; tci < typesToCheck.length; tci++) {
		...
		for (var i in EDDLTD.metaDataSegmentLabels) {
			...
			var s = <any>document.getElementById('sec' + i + 'datatype');
			if (!s) {
				continue;
			}
			var t = s.value;
	...
	for (var i:any=0; i < disTableHeaderLabels.length; i++) {
	...
	// Now that all the content is part of the DOM, we'll call some Javascript routines
	// to initialize the autocomplete fields.

	for (var ae=0; ae < EDDLTD.disambiguationElements.length; ae++) {
		var record = EDDLTD.disambiguationElements[ae];
		var elementName:string = record.e;
		var inObject2:any = document.getElementById(elementName);
	...
	if (tn == "td") {
		...
		if (x != null && y != null) {
			...
			if ((x>1) && (y>1)) {
				...
				if (EDDLTD.activeFlagsData[x][y]) {	// Exists and nonzero
...
generateFormSubmission:function() {
	var dest = <any>document.getElementById("parsedlinesjson");
	if (!dest) {
		return;
	}
	...
	var lineMetaEl = <any>document.getElementById("linemetachoicesjson");
	if (!lineMetaEl) {
		return;
	}
	...
	var disArrayEl = <any>document.getElementById("disambiguationsjson");
	if (!disArrayEl) {
		return;
	}
