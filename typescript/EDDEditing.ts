	...
	if (show) {
		...
		for (var objIndex = 0; objIndex < showList.length; objIndex++) {
			...
			if (oneObj) {
				...
				var elements:any = oneObj.getElementsByTagName('input');
...
//		} else if (element.type == 'hidden') {	// We're leaving hidden elements alone
//			element.value = '';
...
checkFormElements:function(elements, fData) {
	...
	for (var y = 0; y < z; y++) {
		...
		if ((typeof fData[oneName] == "object") && (fData[oneName].constructor == Array)) {	
			oneVal = '';
			for (var i:any=0; i < fData[oneName].length; i++) {
		...
		if (element.type == 'select-one') { 
			var newIndex = -1;
			for (var i:any=0; i < element.length; i++) {
	...
	var z:any = autofillElements.length;
	for (var y = 0; y < z; y++) {
