...
function loadAllProtocols(): void {
    EddRest.loadProtocols({
        ...
        "error": () => {
            showLoadFailed(PROTOCOL_DIV_SELECTOR);
        },
        "request_all": true, // get all result pages
        "wait": () => {
            showWaitMessage(PROTOCOL_DIV_SELECTOR);
        },
        "ordering": "name",
    });
}
...
function lineMetaSuccessHandler(metadataTypes: EddRest.MetadataType[]) {
    ...
    showMetadataTypes(
        LINE_DIV_SELECTOR,
        metadataTypes,
        EddRest.LINE_PROPERTY_META_UUIDS,
    );
}
...
function assayMetaSuccessHandler(metadataTypes: EddRest.MetadataType[]) {
    ...
    showMetadataTypes(
        ASSAY_DIV_SELECTOR,
        metadataTypes,
        EddRest.ASSAY_PROPERTY_META_TYPES,
    );
}
...
function showMetadataTypes(
    ...
    /* TODO: consider merging with showProtocols() above
        IF the back-end MetadataType class gets refactored to use the Unit class
...
function measurementUnitsErrorHandler(
    jqXHR,
    textStatus: string,
    errorThrown: string,
): void {
