...
export class EDDRnaSeq {
    ...
    process_result(result: any): void {
        if (result.python_error) {
            ...
            for (let i = 0; i < result.samples.length; i++) {
                ...
                const fields = $("#sample-fields-0").clone();
                fields.find("#col-label").text(sample.label);
                fields
                    .find("#assay-id")
                    .val(0)
                    .attr("name", "assay-" + i);
            ...
            $("#data-info").text(
                "Table contains counts and RPKMs for " + result.n_genes + " genes.",
            );
        } else {
     ...
     */
    submit_data(): void {
        const form_element = document.getElementById("data-form") as HTMLFormElement;
    ...
    initialize_window(study_id: number): void {
        ...
        $(() => {
            const filedrop_url = "/study/" + this._study_id + "/import/rnaseq/parse/";
            $("#file-name").on("change", () => {
                $("#data-field").text("");
                $("#data-info").text("");
                $("#data-table").val("");
