...
changedMasterProtocol:function() {
	...
	var masterProtocolEl = <HTMLInputElement>document.getElementById("masterProtocol");
	...
	// We're gonna recreate this pulldown's content to list the Assays specific to the selected Protocol
...
processImportSettings:function() {
	...
	var mainRadioSTD = <HTMLInputElement>document.getElementById("stdlayout");
	var mainRadioTR = <HTMLInputElement>document.getElementById("trlayout");
	var mainRadioPR = <HTMLInputElement>document.getElementById("prlayout");
	var mainRadioMDV = <HTMLInputElement>document.getElementById("mdvlayout");
	var ignoreGapsEl = <HTMLInputElement>document.getElementById("ignoreGaps");
	var transposeEl = <HTMLInputElement>document.getElementById("transpose");
...
pastedRawData:function() {
	...
	window.setTimeout(function() {
		...
		var textData = <HTMLInputElement>document.getElementById("textData");
...
parseAndDisplayText:function() {
    ...
    for (var y in unfilteredrows) {
        var cells:any = unfilteredrows[y].split(delimiter);
	...
	if (iMode == 'std' || iMode == 'tr' || iMode == 'pr') {
		...
		for (var ri = 0; ri < rows.length; ri++) {
			var cells:any = rows[ri].split(delimiter);
		...
		if (!EDDATD.Grid.userClickedOnTranspose) {
			...
			for (var ai = 0; ai < arraysToAnalyze.length; ai++) {
				...
				var score = 0;
				if (!oneArray) {	// null check
					continue;
				}
				if (oneArray.length < 1) {	// This would get a score of 0 anyway - and also cause a division by zero error
					continue;
				}
				var previous = null;
				...
				for (var i = 0; i < oneArray.length; i++) {
					...
					if (current != null) {
						...
						if (isNaN(parseFloat(current))) {	// If it won't parse, call it null
							current = null;
						}
	...
	// We meed at least 2 rows and columns for MDV format to make any sense
		...
		for (var y in rows) {
			var cells:any = rows[y].split(delimiter);
		...
		for (var c in compoundsSeen) {
			...
			origRowIndexes.sort(function(a,b){return a - b}); // Sort ascending

			// Then we'll run through the set of columnLabels we accumulated above,
			// assembling a full carbon marking number for each one,
	...
	// The checkboxes that go along the top above the data columns
...
changedAMasterPulldown:function() {
	var masterAssayEl = <any>document.getElementById("masterAssay");
	if (masterAssayEl == null) {
		return;
	}
	var masterLineSpan = document.getElementById('masterLineSpan');
