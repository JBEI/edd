...
module StudyD {
	...
	export class GenericFilterSection  {
		...
		// A dictionary resolving a record ID (assay ID, measurement ID) to an array.
		// Each array contains the integer identifiers of the unique values that apply to that record.
		filterHash:any;
		// Dictionary resolving the filter value integer identifiers to HTML Input Element checkboxes.
		checkboxes:any;
		// Dictionary used to compare checkboxes with a previous state to determine whether an update is required.
		// Values are 'C' for checked, 'U' for unchecked, and 'N' for not existing at the time.
		// ('N' can be useful when checkboxes are removed from a filter due to the back-end data changing.)
		...
		createContainerObjects():void {
	        ...
	        $(sBox).attr({ 'id': sBoxID, 'name': sBoxID, 'placeholder': this.sectionTitle, 'size': 14})
	            .addClass('searchBox')
	...
	export function prepareFilteringSection() {
		...
		for (var i = 0; i < EDDData.MetaDataTypeIDs.length; i++) {	// This is in alphabetical order by name
		...
		this.allFilteringWidgets = assayFilters.concat(
			this.metaboliteFilteringWidgets, this.proteinFilteringWidgets, this.geneFilteringWidgets
		);
	...
	export function prepareAfterLinesTable() {
		...
		if (filterTable) {
			...
			filterTable.addEventListener('keydown', (e) => StudyD.filterTableKeyDown(e), false);
		}
...
//		console.log(($.now()/1000) + ' requesting gene data');
	...
	export function carbonBalanceColumnRevealedCallback(index:any, spec:DataGridSpecLines, dataGridObj:DataGrid) {
	...
	export function assaysActionPanelShow() {
		...
		for (var i:any=0; i<checkedBoxes.length; i++) {
	...
	export function remakeMainGraphArea(force?:boolean) {
		...
		// Walk down the filter widget list.  If we encounter one whose collective checkbox state has changed
		// since we last made this walk, then a redraw is required.
		// Note that we should not skip this loop, even if we already know a redraw is required,
		// since the call to anyCheckboxesChangedSinceLastInquiry sets internal state in the filter widgets
		// that we will use next time around.
		for (var i=0; i < this.allFilteringWidgets.length; i++) {
		...
		var pointsDisplayedSpan = <any>document.getElementById('pointsDisplayedSpan');
		...
		if (dataPointsDisplayed != dataPointsTotal) {
			pointsDisplayedSpan.appendChild(
				document.createTextNode(dataPointsDisplayed + " points displayed (out of " + dataPointsTotal + ")"));
		} else {
			pointsDisplayedSpan.appendChild(
				document.createTextNode(dataPointsDisplayed + " points displayed"));			
		}
...
//		}
	...
	export function onClickedMetabolicMapName() {
		new StudyMetabolicMapChooser(EDDData.currentUserID, EDDData.currentStudyID, false, (err, metabolicMapID, metabolicMapName, finalBiomass:number) => {
			if ( err == null ) {
...
class DataGridSpecLines extends DataGridSpecBase {
    ...
    private loadCarbonSource(index:any):string {
    ...
    private loadCarbonSourceLabeling(index:any):string {
        // ensure carbon source ID(s) exist on line, ensure at least one source ID, ensure first ID is known carbon source, uppercase label or ?
	...
	defineColumnSpec():DataGridColumnSpec[] {
		var leftSide:DataGridColumnSpec[] = [
            ...
            // The Carbon Balance cells are populated by a callback, triggered when they are first displayed
	...
	defineColumnGroupSpec():DataGridColumnGroupSpec[] {
		...
		var metaDataColGroups:DataGridColumnGroupSpec[] = this.metaDataIDsUsedInLines.map((id, index) => {
			var mdType = EDDData.MetaDataTypes[id];
	...
	clickHandler=(e)=>{
		...
		if (this.checkBoxElement.checked) {
			...
			if (!StudyD.biomassCalculation || StudyD.biomassCalculation == -1 ) {
				...
				new FullStudyBiomassUI(EDDData.currentUserID, EDDData.currentStudyID, (err, finalMetabolicMapID, finalMetabolicMapFilename, finalBiomass:number) => {
...
class DataGridAssays extends DataGrid {
	...
	remakeGraphArea() {
		...
		for (var x = 0; x < ids.length; x++) {
			var assayID = ids[x];
			var assayRecord = EDDData.Assays[assayID];
			if (assayRecord.dis) { continue; }	// We wont display items from disabled Assays on the graph
			var lineID = assayRecord.lid;
			var lineRecord = EDDData.Lines[lineID];
	    	var pid = assayRecord.pid;
			var fn = [lineRecord.n, EDDData.Protocols[pid].name, assayRecord.an].join('-');
...
class DataGridSpecAssays extends DataGridSpecBase {	
	...
	refreshIDList() {
		...
		for (var assayID in EDDData.Assays) {
			...
			if (!assayRecord.mea_c) {	// No measurements of any kind?
				continue;
			}
	...
	onDataReset(dataGrid:DataGrid):void {
		...
		if (this.measuringTimesHeaderSpec) {
			if (this.measuringTimesHeaderSpec.element) {
				var childDiv:any = this.measuringTimesHeaderSpec.element.firstChild;
				if (childDiv) {
	...
	findMetaDataIDsUsedInAssays() {
		...
		for (var x = 0; x < ids.length; x++) {
			...
			if (!record.hasOwnProperty('md')) {	continue;	}
	...
	defineHeaderSpec():DataGridHeaderSpec[] {
		...
		this.measuringTimesHeaderSpec = new DataGridHeaderSpec(5 + metaDataHeaders.length, 'hAssaysMTimes'+this.protocolID, {
                'name': 'Measuring Times', 'headerRow': 2});
	...
	defineColumnGroupSpec():DataGridColumnGroupSpec[] {
		...
		var metaDataColGroups:DataGridColumnGroupSpec[] = this.metaDataIDsUsedInAssays.map((id, index) => {
			var mdType = EDDData.MetaDataTypes[id];
            return new DataGridColumnGroupSpec(mdType.name);
		});
	...
	onInitialized(dataGrid:DataGridAssays):void {
		...
		if (this.graphAreaHeaderSpec) {
			if (this.measuringTimesHeaderSpec.element) {
				this.graphAreaHeaderSpec.element.innerHTML =
					'<div id="pro'+p+'graph" style="width: 98%; height: 240px; padding: 0px; margin: 5px 0px;"></div>';
