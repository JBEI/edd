...
if [ -r /var/edd/ssl/dhparam.pem ]; then
    echo "Copying dhparam.pem to Docker TLS volume …"
    cp /var/edd/ssl/dhparam.pem /etc/ssl/edd/dhparam.pem
...
if [ -r /var/edd/ssl/certificate.chained.crt ] \
        && [ -r /var/edd/ssl/certificate.key ]; then
    echo "Copying certificate and key to Docker TLS volume …"
    cp /var/edd/ssl/certificate.chained.crt /etc/ssl/edd/certificate.chained.crt
    cp /var/edd/ssl/certificate.key /etc/ssl/edd/certificate.key
...
if [ -r /var/edd/ssl/trustchain.crt ]; then
    echo "Copying trustchain to Docker TLS volume …"
    cp /var/edd/ssl/trustchain.crt /etc/ssl/edd/trustchain.crt
...
envsubst '' < /usr/local/nginx/nginx.conf > /etc/nginx/nginx.conf
