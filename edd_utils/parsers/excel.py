...
def export_to_xlsx (table, headers=None, title="Exported table",
    file_name=None) :
  """
...
def import_xlsx_tables (file,
  ...
  if (len(ws_results) == 0) :
    raise ValueError("No valid table-like blocks of contiguous cells found "+
...
def _find_table_of_values (worksheet,
    column_labels=None,
    column_search_text=None,
    expect_numeric_data=False,
    followed_by_blank_row=False,
      ...
      if (column_labels is not None) :
        ...
        for cell_index, value in enumerate(row) :
          ...
          if (value.lower() in column_labels) :
            column_indices.append(cell_index)
        ...
        if (len(headers) >= 2) :
          ...
          while (row_index < n_rows) :
            ...
            table.append(row)
          if (len(table) == 0) :
            raise ValueError(("The column labels '%s' were found in the "+
        ...
        for cell_index, value in enumerate(row) :
          ...
          if (isinstance(value, basestring) and
              (column_search_text in value.lower())) :
            headers = row[cell_index:]
            while (row_index < n_rows) :
              row = rows[row_index][cell_index:]
              row_index += 1
        ...
        if (len(headers) > 0) :
          if (len(table) == 0) :
            raise ValueError(("The search text '%s' was found in the "+
              "worksheet, but no recognizable table of values was associated "+
  ...
  if (column_search_text is not None) :
    raise ValueError(("The specified search text '%s' could not be associated "+
  ...
  if (len(possible_tables) > maximum_number_of_tables) :
    raise ValueError("Multiple table-like blocks of cells identified in "+
    ...
    raise ValueError("No table-like blocks of cells could be automatically "+
