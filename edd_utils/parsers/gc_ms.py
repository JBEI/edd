...
class Sample (object) :
  ...
  def get_unrecognized_peaks (self, peak_times=None, rt_tolerance=None,
      rt_ranges=None) :
...
class SampleCollection (object) :
    ...
    for i_sample, sample in enumerate(self.samples) :
      ...
      for i_peak, rt in enumerate(peak_times) :
        ...
        if (n_peaks == 0) :
          ...
          if (n_peaks > 1) :
            print >> err, "WARNING: %d peaks near %.3f for sample %s" %\
  ...
  def show_peak_areas (self, n_expected=None, out=sys.stdout,
      err=sys.stderr) :
    ...
    print >> out, "%12s %s" % ("ID",
      " ".join([ "%12s" % ("Peak %d"%(k+1)) for k in range(len(table[0])-1) ]))
    print >> out, "\n".join(
      [ " ".join([ "%12s" % s for s in row]) for row in table ])

  def show_peak_areas_csv (self, n_expected=None, out=sys.stdout,
      err=sys.stderr) :
    peak_times, bandwidth = self.find_consensus_peaks(
      n_expected=n_expected,
      out=out,
    ...
    table, errors = self.extract_peak_areas(peak_times, bandwidth, err=err)
    print >> out, "\n".join(
      [ ",".join([str(s).strip() for s in row]) for row in table ])
...
class Report (SampleCollection) :
  ...
  def __init__ (self, lines) :
    ...
    while (k < len(lines)) :
      ...
      k += 1
    if (not have_entries) :
      raise ValueError("This content does not appear to be a valid "+
        "ChemStation report.")
...
def import_xlsx_metadata (file, header_key="ID") :
  ...
  while (i_row < n_rows) :
    ...
    i_row += 1
    for cell in row :
      if isinstance(cell.value, basestring) and (header_key in cell.value) :
