...
* `EDD_DEPLOPMENT_ENVIRONMENT` If present, and if the value is `DEVELOPMENT`, then EDD
  will be launched using the non-production `manage.py runserver`, which restarts whenever
  code changes are detected (but isn't suited for production use)
...
* `POSTGRES_DUMP_FILE` points to a database dump file to be loaded into the `postgres` service
  ...
  container is started*, so it's best to provide it an the command line as demonstrated above.
