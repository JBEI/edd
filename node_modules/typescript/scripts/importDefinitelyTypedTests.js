...
function importDefinitelyTypedTest(testCaseName, testFiles, responseFile) {
...
function importDefinitelyTypedTests(definitelyTypedRoot) {
    fs.readdir(definitelyTypedRoot, function (err, subDirectorys) {
        if (err)
            throw err;

        subDirectorys.filter(function (d) {
            return ["_infrastructure", "node_modules", ".git"].indexOf(d) < 0;
        }).filter(function (i) {
            return i.indexOf("sipml") >= 0;
        }).filter(function (i) {
            return fs.statSync(path.join(definitelyTypedRoot, i)).isDirectory();
        }).forEach(function (d) {
            ...
            fs.readdir(directoryPath, function (err, files) {
                ...
                files.map(function (f) {
                    return path.join(directoryPath, f);
                }).forEach(function (f) {
                    if (fileExtensionIs(f, ".ts"))
                        tsFiles.push(f);
                    else if (fileExtensionIs(f, ".tscparams"))
                        paramFile = f;

                    if (fileExtensionIs(f, "-tests.ts"))
                        testFiles.push(f);
                });
                ...
                if (testFiles.length === 0) {
                    ...
                    if (tsFiles.length > 1 && tsFiles.every(function (t) {
                        return fileExtensionIs(t, ".d.ts") && regexp.test(t);
                    })) {
                        tsFiles.forEach(function (filename) {
...
//# sourceMappingURL=importDefinitelyTypedTests.js.map
