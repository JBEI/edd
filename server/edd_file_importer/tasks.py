...
from main.models import SYSTEM_META_TYPES, MeasurementUnit, MetadataType
...
def update_import_status(status, import_uuid, user_pk, notify=None):
    ...
    notify.notify(
        ...
        payload={"status": status, "uuid": import_uuid, "pk": import_.pk},
    )
...
def process_import_file(import_pk, user_pk, requested_status, initial_upload):
    ...
    try:
        ...
        study_url = (
            reverse("main:overview", kwargs={"slug": import_.study.slug})
            if import_
            else ""
        )
        logger.exception(
...
def _verify_status_transition(aggregator, import_, requested_status, user, notify):
    ...
    if requested_status != Import.Status.SUBMITTED:
        ...
        return aggregator.raise_error(
            err_codes.ILLEGAL_STATE_TRANSITION, occurrence=msg
        )

    elif import_.status not in (
        Import.Status.READY,
        Import.Status.ABORTED,
        Import.Status.FAILED,
    ):
        msg = (
            f"Transition from {import_.status} to {Import.Status.SUBMITTED} is not allowed or "
            f"not yet supported"
        )
        return aggregator.raise_error(
            err_codes.ILLEGAL_STATE_TRANSITION, occurrence=msg
...
def submit_import(import_, user_pk, aggregator, notify, run_async):
    ...
    try:
        ...
        mark_import_processing = update_import_status.si(
            Import.Status.PROCESSING, uuid, user_pk
        )
        mark_import_complete = update_import_status.si(
            Import.Status.COMPLETED, uuid, user_pk
        )
        mark_import_failed = update_import_status.si(
            Import.Status.FAILED, uuid, user_pk
        )
...
def build_ui_payload_from_cache(import_pk, user_pk):
    ...
    if found_count != len(parser.mtype_pks):
        ...
        aggregator.raise_errors(
            err_codes.MEASUREMENT_TYPE_NOT_FOUND, occurrences=missing_pks
        )
    ...
    if parser.matched_assays:
        ...
        assay_pk_to_time = verify_assay_times(
            aggregator, assay_pks, parser, assay_time_meta_pk
        )
    required_inputs = compute_required_context(
        category, import_.compartment, parser, assay_pk_to_time
    )
    payload = build_summary_json(
        import_, required_inputs, import_records, unique_mtypes, hour_units.pk
    )
    notify = RedisBroker(user)
