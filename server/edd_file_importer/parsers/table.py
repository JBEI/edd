...
class TableParser(object):
    ...
    def parse_excel(self, file):
        ...
        if not wb.worksheets:
            ...
            msg = (
                f'Only the first sheet in your workbook, "{sheet_name}", was processed.  '
                f"All other sheets will be ignored."
            )
            self.aggregator.add_warning(
                FileParseCodes.IGNORED_WORKSHEET, occurrence=msg
            )
    ...
    def _parse(self, rows_iter):
        ...
        for row_index, cols_list in enumerate(rows_iter):
            ...
            if not self.column_layout:
                self.column_layout = self._parse_col_layout(
                    cols_list, row_index, obs_required_cols
                )
        ...
        if not self.column_layout:
            ...
            self.aggregator.add_error(
                FileParseCodes.MISSING_REQ_COL_HEADER, ", ".join(self.req_cols)
            )
    ...
    def _parse_col_layout(self, row, row_index, obs_required_cols):
        ...
        if obs_required_cols:
            ...
            importer.raise_errors(
                FileParseCodes.MISSING_REQ_COL_HEADER, occurrences=missing_cols
            )
        else:
            importer.add_warnings(
                FileParseCodes.IGNORED_VALUE_BEFORE_HEADERS, non_header_vals
            )
    ...
    def _parse_num(self, token, col_name, row_index, col_index):
        try:
            ...
            self.aggregator.add_error(
                FileParseCodes.INVALID_VALUE,
                subcategory=self.obs_col_name(col_name),
                occurrence=self.cell_content_desc(token, row_index, col_index),
            )
            return None
...
class GenericImportParser(TableParser):
    ...
    def _parse_row(self, cells_list, row_index):
        ...
        line_or_assay_name = self._parse_and_verify_val(
            line_or_assay_name, row_index, "Line Name"
        )
    ...
    def has_all_times(self):
        ...
        return (
            True
        )  # overrides default False.  Time is a required column in this format.
    ...
    def has_all_units(self):
        ...
        return (
            True
        )  # overrides default False.  Units is a required column in this format
