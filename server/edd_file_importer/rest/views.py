...
from ..tasks import (
    ...
    build_ui_payload_from_cache,
    process_import_file,
)
from ..utilities import CommunicationError, EDDImportError, build_err_payload
...
def _build_simple_err_response(
    self, category, summary, status=codes.internal_server_error, detail=None
):
    payload = {
        "errors": [
            {
...
class StudyImportsViewSet(
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        try:
            ...
            if requested_status:
                attempt_status_transition(
                    import_, requested_status, self.request.user, run_async=True
                )
    ...
    def _reprocess_file(self, import_, request, reupload, user_pk):
        ...
        process_import_file.delay(
            import_.pk, user_pk, request.data.get("status", None), initial_upload=False
        )
        ui_payload = {"uuid": import_.uuid, "pk": import_.pk, "status": import_.status}
        return JsonResponse(
            ui_payload, encoder=JSONEncoder, status=codes.accepted, safe=False
        )
    ...
    def _save_context(self, import_, request, study_pk, import_pk, user_pk):
        ...
        """
        logger.info(
            f"Updating import context for study {study_pk}, import {import_pk}, "
            f"user {user_pk}"
        )
        update_params = [
            param
            for param in ("x_units", "y_units", "compartment")
