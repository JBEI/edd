...
from main.importer.parser import ImportFileTypeFlags, guess_extension
...
class ImportContext:
    ...
    def __init__(self, import_, aggregator, user):
        ...
        self.assay_time_metatype = MetadataType.objects.get(
    ...
    def _get_parser_instance(self, aggregator):
        ...
        if i < 0:
            aggregator.add_error(
                FileParseCodes.PARSER_NOT_FOUND, occurrence=parser_class_name
            )
...
class ImportFileHandler(ErrorAggregator):
    ...
    def process_file(self, initial_upload):
        ...
        required_inputs = compute_required_context(
            category, compartment, parser, assay_pk_to_time
        )
        ...
        import_records = self.cache_resolved_import(
            import_.uuid, parser, matched_assays, initial_upload
        )
        ...
        payload = build_summary_json(
            import_, required_inputs, import_records, unique_mtypes, import_.x_units_id
        )
        msg = _('Your file "{file_name}" is ready to import').format(
            file_name=file_name
        )
        self.notify.notify(msg, tags=["import-status-update"], payload=payload)
    ...
    def _parse_file(self):
        ...
        if file_extension not in (ImportFileTypeFlags.EXCEL, ImportFileTypeFlags.CSV):
            self.raise_error(
                FileParseCodes.UNSUPPORTED_FILE_TYPE, occurrence=file_extension
            )
    ...
    def _verify_line_or_assay_match(self, line_or_assay_names, lines):
        ...
        if found_count:
            ...
            logger.info(
                f"Matched {found_count} of {input_count} {model} names "
                f"from the file"
            )
            ...
            if found_count != input_count:
                if found_count < input_count:
                    ...
                    err_code = (
                        ...
                        if lines
                        else FileProcessingCodes.UNMATCHED_ASSAY_NAME
                    )
                    ...
                    err_code = (
                        ...
                        if lines
                        else FileProcessingCodes.DUPLICATE_ASSAY_NAME
                    )
    ...
    def _verify_measurement_types(self, parser):
        ...
        types = (
            f": {parser.unique_mtypes}" if types_count <= 10 else f"{types_count} types"
        )
        logger.debug(
            f'Verifying MeasurementTypes for category "{category.name}"=> '
    ...
    def cache_resolved_import(self, import_id, parser, matched_assays, initial_upload):
        ...
        import_records = (
            OrderedDict()
        )  # maintain the order of items from the file for debugging
        for index, parse_record in enumerate(parser.series_data):
            ...
            assay_or_line_pk = self.cache.loa_name_to_pk.get(
                parse_record.line_or_assay_name
            )
            mtype = (
                cache.mtype_name_to_type[parse_record.mtype_name]
                if parse_record.mtype_name
                else None
            )
            unit = (
                cache.unit_name_to_unit[parse_record.units_name]
                if parse_record.units_name
                else None
            )
    ...
    def _build_import_record(self, parse_record, matched_assays):
        ...
        mtype = (
            self.cache.mtype_name_to_type[parse_record.mtype_name]
            if parse_record.mtype_name
            else None
        )
        unit = (
            self.cache.unit_name_to_unit[parse_record.units_name]
            if parse_record.units_name
            else None
        )
        import_record = {
        ...
        if matched_assays:
            ...
            assays = Assay.objects.filter(
                line_id=line_pk, protocol_id=protocol.pk
            ).values_list("name")
            if assays:
                self.raise_errors(
                    FileProcessingCodes.MERGE_NOT_SUPPORTED, occurrences=assays
                )
    ...
    def _record_record_clash(
        self, loa_name, import_time, import_record, parse_record, mtype
    ):
        occurrences = []
