...
class Session(SessionApi):
    ...
    def __init__(
        self, timeout=DEFAULT_TIMEOUT, verify_ssl_cert=VERIFY_SSL_DEFAULT, auth=None
    ):
    ...
    def _add_pagination_params(self, url, **kwargs):
        ...
        if found_in_url:
            ...
            if url_param_val != updated_value:
                ...
                logger.debug(
                    "......Original URL: %s, Updated URL: %s" % (original_url, url)
                )
        ...
        if not params:
            ...
            if existing_value != self.result_limit:
                logger.warning(
                    'An existing request parameter named "%(param)s" was present. '
...
class PagedResult(object):
    ...
    def __str__(self):
        return (
            "<PagedResult count=%(count)d, next_page=%(next)s, previous_page=%(prev)s"
            % {
