...
from jbei.rest.clients.ice.api import ICE_URL, VERIFY_SSL_DEFAULT
...
class IceTestStub(IceApi):
    ...
    def __init__(
        ...
        super(IceTestStub, self).__init__(
            auth,
            base_url=base_url,
    ...
    def get_entry(self, entry_id, suppress_errors=False):
        ...
        if self._query_num != self._fail_on_query_num:
            logger.debug(
                "On query %d ...waiting to fail on #%d.."
                % (self._query_num, self._fail_on_query_num)
            )
            return super(IceTestStub, self).get_entry(
                entry_id, suppress_errors=suppress_errors
            )
