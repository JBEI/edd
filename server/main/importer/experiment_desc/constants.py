...
FOLDER_NOT_FOUND = (
    "Folder(s) weren't found found in ICE, or your account doesn't have "
...
ZERO_REPLICATES = (
    "Zero replicates are not allowed. If no lines are desired, remove row(s) from "
    "the file."
)
...
UNMATCHED_ASSAY_COL_HEADERS_KEY = (
    "Invalid column header(s) (Unmatched assay metadata suffix)"
)
...
DELIMETER_NOT_ALLOWED_VALUE = (
    "Semicolon character isn't allowed within part numbers. Use commas "
...
FORBIDDEN_FOLDER_KEY = (
    "Unable to determine how many lines to create, since folder "
...
LINE_META_CAPITALIZATION_ONLY_DIFFERENCE = (
    "Found some line metadata types that differ only by "
    "case. Case-insensitive matching in parsing code will "
    "arbitrarily choose one"
)
ASSAY_META_CAPITALIZATION_ONLY_DIFFERENCE = (
    "Found some assay metadata types that differ only by "
    "case. Case-insensitive matching in parsing code will"
    " arbitrarily choose one"
)
...
UNMATCHED_PART_NUMBER = (
    "Unmatched part number(s). This indicates a coding error in EDD."
)
...
PART_NUMBER_PATTERN_UNMATCHED_WARNING = (
    "Part number(s) didn't match the expected pattern"
)
...
INCONSISTENT_FOLDERS = (
    "Inconsistent ICE folders.  Folders must be the same for all rows"
)
...
STRAINS_REQUIRED_TITLE = (
    "ICE strains are required for combinatorial line creation, but couldn't "
    "be found in ICE"
)
STRAINS_ACCESS_REQUIRED_FOR_COMBO = (
    "Missing ICE read permission for parts required as input "
    "for combinatorial line creation"
)

STRAINS_REQUIRED_FOR_COMBO = (
    "ICE lookup failed for one or more entries, but entries are "
    "required for combinatorial line creation"
)
STRAINS_REQUIRED_FOR_NAMES = (
    "ICE lookup failed for one or more entries, but entry names are "
    "required as input to computing line names"
)

STRAINS_ACCESS_REQUIRED_FOR_NAMES = (
    "Missing ICE read permission for part(s) required as input "
...
###################################################################################################
USER_CREATED_ICE_PART_ERRORS = (PART_NUMBER_NOT_FOUND, FORBIDDEN_PART_KEY)
...
USER_CREATED_ICE_FOLDER_ERRORS = (FOLDER_NOT_FOUND, FORBIDDEN_FOLDER_KEY)
...
ERROR_PRIORITY_ORDER[SYSTEMIC_ICE_ACCESS_ERROR_CATEGORY] = (
    ICE_NOT_CONFIGURED,
...
WARNING_PRIORITY_ORDER[POSSIBLE_USER_ERROR_CATEGORY] = (
    PART_NUMBER_PATTERN_UNMATCHED_WARNING,
)
...
WARNING_PRIORITY_ORDER[SYSTEMIC_ICE_ACCESS_ERROR_CATEGORY] = (
    GENERIC_ICE_RELATED_ERROR,
)
