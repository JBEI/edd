...
class ExperimentDescriptionTests(TestCase):
    ...
    def _run_upload(self, name):
        ...
        response = self.client.post(
            reverse("main:describe", kwargs=self.target_kwargs), data={"file": upload}
        )
        return response
    ...
    # bump up timeout length so Jenkins build image can properly execute
    @override_settings(ICE_REQUEST_TIMEOUT=(30, 30))
    def test_missing_strain(self):
        name = "ExperimentDescription_missing_strain.xlsx"
        response = self._run_upload(name)
        self.assertEqual(response.status_code, codes.bad_request, f"{response.content}")
