...
class LineMetadataChecker(object):
    """Holds state for checking Line metadata is handled as expected."""

    def __init__(self, testcase, line, lookup, related_pks, many_related_pks):
        self.testcase = testcase
        self.line = line
        self.lookup = lookup
        self.related_pks = related_pks
        self.many_related_pks = many_related_pks

    def _test_line_metadata_item(self, meta_pk, exp_meta_val, line):
        ...
        meta_type = self.lookup.line_meta_types[meta_pk]

        obs_val = line.metadata_get(meta_type)
        ...
        if meta_pk in self.many_related_pks:
            related_object = True
            many_related_obj = True
            obs_val = line_attr.values_list("pk", flat=True).order_by("pk")
        ...
        elif meta_pk in self.related_pks:
            related_object = True
        ...
        if related_object:
            self.testcase.assertEqual(line.metadata.get(meta_pk), None)
        ...
        self.testcase.assertEqual(
            exp_meta_val,
            obs_val,
            f'MetadataType "{meta_type.type_name}" (pk={meta_pk})',
        )
...
class CombinatorialCreationTests(TestCase):
    ...
    def build_ui_json(
        self, strain1, strain2, cs_glucose, cs_galactose, growth_temp_meta
    ):
        """
        ...
        gui_mockup_example = {
            "name_elements": {
                ...
                "abbreviations": {
                    strains_name: {strain1.name: 58, strain2.name: 27},
                    carbon_src_name: {cs_glucose.name: "GLU", cs_galactose.name: "GAL"},
                },
            },
            "replicate_count": 3,
            "common_line_metadata": {growth_temp_meta.pk: 30},  # degrees C
        ...
        if not self._OMIT_ASSAYS_FROM_TEST:
            gui_mockup_example["protocol_to_combinatorial_metadata"] = {
                ...
                self.metabolomics.pk: {cache.assay_time_mtype.pk: [4, 6]},  # hours
            }
    ...
    def test_ed_file_parse_err_detection(self):
        ...
        classname = (
            "main.importer.experiment_desc.importer.CombinatorialCreationImporter"
        )
        with patch(classname) as MockImporter:
            ...
            exp_calls = [
                ...
                call(
                    ...
                    UNMATCHED_ASSAY_COL_HEADERS_KEY,
                    '"Tomperature" (col H)',
                ),  # TODO: use a different suffix for clarity!
                call(BAD_FILE_CATEGORY, DUPLICATE_LINE_ATTR, '"Line Name" (col I)'),
                call(
                    BAD_FILE_CATEGORY, DUPLICATE_LINE_ATTR, '"Replicate Count" (col J)'
                ),
                call(
                    BAD_FILE_CATEGORY,
                    DUPLICATE_ASSAY_METADATA,
    ...
    def test_add_line_combos_use_case(self):
        ...
        ###########################################################################################
        ui_json_output = self.build_ui_json(
            strain1, strain2, cs_glucose, cs_galactose, growth_temp_meta
        )
        ...
        strains_by_pk = {strain1.pk: strain1, strain2.pk: strain2}
        cache.strains_by_pk = strains_by_pk

        study = Study.objects.create(name="Unit Test Study")

        exp_assay_suffixes, exp_assay_metadata = self.build_assay_data(
            expected_line_names
        )
    ...
    def _assert_combinatorial_names(
        self,
        expected_line_names,
        ...
        for line_name in expected_line_names:
            if expected_protocols_to_assay_suffixes:
                for (
                    protocol_pk,
                    exp_suffixes,
                ) in expected_protocols_to_assay_suffixes.items():
                ...
                self.assertFalse(
                    naming_results.line_to_protocols_to_assays_list.get(
                        line_name, False
                    )
                )
    ...
    def _test_combinatorial_creation(
        ...
        # pulled loop into its own subroutine
        self._assert_combinatorial_names(
            expected_line_names,
            expected_protocols_to_assay_suffixes,
            naming_results,
            cache,
        ...
        for line_index, created_line in enumerate(creation_results.lines_created):
            ...
            protocol_to_planned_assay_names = naming_results.line_to_protocols_to_assays_list.get(
                created_line.name, {}
            )

            self.assertEqual(
                len(protocol_to_assays_list), len(protocol_to_planned_assay_names)
            )
    ...
    def _test_line_metadata(
        self, line, exp_metadata, cache, related_object_mtypes, many_related_obj_mtypes
    ):
        if not exp_metadata:
        ...
        checker = LineMetadataChecker(
            self, line, cache, exp_related_obj_meta_pks, exp_many_related_obj_meta_pks
        )
        # loop over expected metadata for this line
        for meta_pk, exp_meta_val in exp_metadata.items():
            checker._test_line_metadata_item(meta_pk, exp_meta_val, line)

    def _test_combinatorial_input(
        self,
        study,
        source_input,
        expected_line_names,
        expected_assay_suffixes,
        edd_strains_by_ice_id=None,
        exp_line_metadata=None,
        exp_assay_metadata=None,
        is_excel_file=False,
    ):
        """
        ...
        edd_strains_by_ice_id = (
            edd_strains_by_ice_id if edd_strains_by_ice_id is not None else {}
        )
        ...
        for combo in combinatorial_inputs:
            unique_strain_ids = combo.get_related_object_ids(
                cache.strains_mtype.pk, unique_strain_ids
            )
        missing_strain_ids = [
            strain_id
            for strain_id in unique_strain_ids
            if strain_id not in edd_strains_by_ice_id
        ]
        if missing_strain_ids:
            self.fail(
                "Strain identifiers provided in source_input were not found in "
        ...
        if edd_strains_by_ice_id:
            ...
            for input_item in combinatorial_inputs:
                input_item.replace_ice_ids_with_edd_pks(
                    edd_strains_by_ice_id, ice_parts_by_id, cache.strains_mtype.pk
                )
        self.assertEqual(
        ...
        if importer.warnings:
            self.fail(
                "Warnings occurred during input parsing: %s" % str(importer.warnings)
            )
        ...
        return self._test_combinatorial_creation(
            ...
            combinatorial_inputs[
                0
            ],  # TODO: hard-coded index with multiples possible (though
            # currently unused)
            importer,
            options,
            expected_line_names,
    ...
    def _test_assay_names(
        self, line_name, naming_results, protocol_pk, expected_suffixes, protocols_by_pk
    ):
        """
    ...
    def test_advanced_experiment_description_xlsx(self):
        ...
        expected_line_metadata = {
            line_name: {str(media_mtype.pk): "LB"} for line_name in expected_line_names
        }
