...
class EDDAccountAdapter(DefaultAccountAdapter):
    ...
    def get_email_confirmation_url(self, request, emailconfirmation):
        ...
        url = super(EDDAccountAdapter, self).get_email_confirmation_url(
    ...
    def pre_social_login(self, request, sociallogin):
        ...
        if qs.exists():
            ...
            messages.error(
                request,
                _(
                    "A %(provider)s account already exists for %(email)s. Please log in with that "
    ...
    def authenticate(self, request, username, password, **kwargs):
        ...
        if user and user.email:
            try:
                ...
                logger.exception(
                    "Failed to check or update email verification from LDAP!"
                )
        return user
