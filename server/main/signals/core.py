...
def line_removed(sender, instance, **kwargs):
    ...
    if check_ice_cannot_proceed():
        return
    if not (
        hasattr(instance, "pre_delete_study")
        and hasattr(instance, "pre_delete_strain_ids")
    ):
        return
    ...
    with transaction.atomic(savepoint=False):
        ...
        post_delete_strain_ids = set(
            edd_models.Strain.objects.filter(study_strains)
            .distinct()
    ...
    partial = functools.partial(
        submit_ice_unlink, instance.pre_delete_study, removed_strains
    )
    connection.on_commit(partial)
...
def strain_removed(line, pk_set):
    ...
    with transaction.atomic(savepoint=False):
        remove_on_commit = {
            ...
            if edd_models.Line.objects.filter(strains=pk, study=line.study.pk).count()
            == 0
        }
    connection.on_commit(
        functools.partial(submit_ice_unlink, line.study, remove_on_commit)
    )
...
def submit_ice_unlink(study, removed_strains):
    ...
    for strain in removed_strains:
        try:
            unlink_ice_entry_from_study.delay(
                settings.ICE_ADMIN_ACCOUNT, strain, study.pk
            )
