...
_LINE_DESCRIPTION_COL_PATTERN = re.compile(
    r"^\s*%s\s*$" % LINE_DESCRIPTION_COL_REGEX, re.IGNORECASE
)
_STRAIN_IDS_SINGULAR_COL_PATTERN = re.compile(
    r"^\s*%s\s*$" % STRAIN_IDS_COL_LABEL, re.IGNORECASE
)
_STRAIN_IDS_PLURAL_COL_PATTERN = re.compile(
    r"^\s*%s\s*\(s\)$" % STRAIN_IDS_COL_LABEL, re.IGNORECASE
)
_REPLICATE_COUNT_COL_PATTERN = re.compile(
    r"^\s*%s\s*$" % REPLICATE_COUNT_COL_REGEX, re.IGNORECASE
)
###################################################################################################
    ...
    def parse(self, raw_value_str):
        ...
        if match:
            ...
            parser.max_fractional_time_digits = max(
                parser.max_fractional_time_digits, fractional_digits
            )
            return number_value
        raise ValueError(
...
class ColumnLayout:
    ...
    def has_assay_metadata(self, upper_protocol_name, metadata_pk):
        """
        Tests whether any columns have been detected so far that store a specific
        (Protocol, MetadataType) pair.

        :param upper_protocol_name:
        :param metadata_pk:
        :return:
        """
        pairs = self.col_index_to_assay_data.values()
        return any(
            True
            for protocol, metatype in pairs
            if upper_protocol_name == protocol.name.upper()
            and metadata_pk == metatype.pk
        )
    ...
    def register_assay_meta_col(self, col_index, protocol, metatype, is_combinatorial):
        ...
        col_letter = get_column_letter(col_index + 1)
        logger.debug(
            f'Column {col_letter} matches protocol "{protocol.name}", assay metadata type'
            f' "{metatype.type_name}"'
        )
    ...
    def register_line_meta_col(self, col_index, line_mtype, is_combinatorial=False):
        col_letter = get_column_letter(col_index + 1)
        logger.debug(
            f"Column {col_letter} matches line metadata type {line_mtype.type_name}"
        )
    ...
    def is_known_column(self, std_content, col_index, cache, importer, cell_content):
        seen_duplicate_col = False
        if _LINE_NAME_COL_PATTERN.match(std_content):
            seen_duplicate_col = self.line_name_col is not None
            if not seen_duplicate_col:
                self.line_name_col = col_index
        elif _LINE_DESCRIPTION_COL_PATTERN.match(std_content):
            seen_duplicate_col = self.line_description_col is not None
            if not seen_duplicate_col:
                self.line_description_col = col_index
        elif _STRAIN_IDS_SINGULAR_COL_PATTERN.match(std_content):
            self.strain_ids_col = col_index
            self.register_line_meta_col(col_index, cache.strains_mtype, False)
        elif _STRAIN_IDS_PLURAL_COL_PATTERN.match(std_content):
            self.strain_ids_col = col_index
            self.register_line_meta_col(col_index, cache.strains_mtype, True)
        elif _REPLICATE_COUNT_COL_PATTERN.match(std_content):
            seen_duplicate_col = self.replicate_count_col is not None
            if not seen_duplicate_col:
                self.replicate_count_col = col_index
        else:
            return False

        if seen_duplicate_col:
            importer.add_error(
                BAD_FILE_CATEGORY,
                DUPLICATE_LINE_ATTR,
    ...
    def get_line_name(self, line_metadata, replicate_num):
        ...
        for line_metadata_col in layout.combinatorial_line_col_order():
            ...
            if (not included_base_name) and (layout.line_name_col < line_metadata_col):
                ...
                name_elts.append(
                    self.base_line_name.replace(" ", self.space_replacement)
                )
            ...
            if line_meta_pk in cache.related_object_mtypes:
                ...
                segment = self.build_related_objects_name_segment(
                    line_metadata, line_meta_pk, related_obj_field, col_letter
                )
    ...
    def get_assay_name(self, line, protocol_pk, assay_metadata):
        ...
        try:
            ...
            for assay_metadata_col in layout.combinatorial_assay_col_order():
                col_protocol, assay_meta_type = layout.col_index_to_assay_data[
                    assay_metadata_col
                ]
                ...
                logger.debug(
                    f"Inspecting combinatorial assay metadata column {col} for protocol "
...
class ExperimentDescFileParser(CombinatorialInputParser):
    ...
    def parse_excel(self, stream) -> list[CombinatorialDescriptionInput]:
        ...
        :param stream: the stream (a.k.a. file-like-object) open in binary mode to load the
            workbook from, or a string
        :return: the list of combinatorial line/assay creations to perform, one per row in the file
    ...
    def _preparse(
        ...
        # flush log message for any trailing empty rows
        self._non_empty_row(row_index)

        return parsed_row_inputs

    def _prepopulate_fractional_time(self, parsed_row_inputs):
        for row_input in parsed_row_inputs:
            row_input.fractional_time_digits = self.max_fractional_time_digits

    def _non_empty_row(self, row_index):
        # log the end of each range of empty rows as its end is reached
        if self.first_empty_row is not None:
            if row_index > self.first_empty_row + 1:
                suffix = "rows {self.first_empty_row + 1}-{row_index + 1}"
            else:
                suffix = f"row {self.first_empty_row}"
            logger.info(f"Ignored empty {suffix}")
            self.first_empty_row = None

    def _remove_non_multiple_combo_columns(self, parsed_row_inputs):
        ...
        # column(s)
        for col_index in self.column_layout.combinatorial_col_indices:
            meta_values = set()
            multivalued_row = False

            line_mtype_pk = self.column_layout.col_index_to_line_meta_pk.get(col_index)
            ...
            if not meta_values or not multivalued_row:
                self.column_layout.combinatorial_col_indices.remove(col_index)

    def _validate_predictable_names(self, parsed_row_inputs):
        ...
        # problem early.
        if self.column_layout.combinatorial_col_indices:
            ...
            for row in parsed_row_inputs:
                ...
                cell_nums.append(
                    cell_coords(row.row_number - 1, self.column_layout.line_name_col)
                )
    ...
    def _validate_registry_names(self, parsed_row_inputs):
            ...
            if index > 0:
                for folder_id in input_folders:
                    if folder_id not in unique_folder_ids:
                        ...
                        if set(current_filters) != set(
                            self.folder_id_to_filters[folder_id]
                        ):
                            self.importer.add_error(
                                INVALID_FILE_VALUE_CATEGORY,
                                INCONSISTENT_FILTERS,
    ...
    def _parse(self, rows_iter) -> list[_ExperimentDescriptionFileRow]:
        ...
        self._prepopulate_fractional_time(parsed_row_inputs)
        self._remove_non_multiple_combo_columns(parsed_row_inputs)
        self._validate_predictable_names(parsed_row_inputs)
        self._validate_registry_names(parsed_row_inputs)
        ...
        for col_index, cell in enumerate(row):
            ...
            if not layout.is_known_column(
                ...
                if not line_metadata_type:
                    ...
                    self.importer.add_error(
                        BAD_FILE_CATEGORY, INVALID_COLUMN_HEADER, cell
                    )
    ...
    def _parse_assay_metadata_header(
        self, layout, upper_content, original_content, col_index
    ):
        """
        ...
        for upper_protocol_name, protocol in self.protocols_by_name.items():
            if upper_content.startswith(upper_protocol_name):
                ...
                for upper_type_name, assay_metadata_type in assay_items:
                    suffix_meta_type, is_combinatorial = self._detect_assay_meta_type(
                        assay_meta_suffix, upper_type_name, assay_metadata_type
                    )
                    if suffix_meta_type:
                        break
                ...
                if suffix_meta_type:
                    ...
                    # if we see it, log an error -- no clear/automated way for us to resolve
                    #  which column has the correct values!
                    if layout.has_assay_metadata(
                        upper_protocol_name, suffix_meta_type.pk
                    ):
                        self.importer.add_error(
                            BAD_FILE_CATEGORY,
                            DUPLICATE_ASSAY_METADATA,
                            header_desc(original_content, col_index),
                        )
                    layout.register_assay_meta_col(
                        col_index, protocol, suffix_meta_type, is_combinatorial
                    )
                ...
                # assumed by the file format.
                    ...
                    logger.debug(
                        f"Column header suffix {value} didn't match known metadata types"
                    )
                    self.importer.add_error(
                        BAD_FILE_CATEGORY, UNMATCHED_ASSAY_COL_HEADERS_KEY, value
                    )
                    return True
    ...
    def _detect_assay_meta_type(self, suffix, upper_type_name, assay_meta_type):
        ...
        :param suffix: the column header suffix to test
        :param upper_type_name: upper-case name of the assay metadata type to test
        :param assay_meta_type: the assay metadata type
        :return: a tuple of (MetadataType, bool), where the first element is the matching
            MetadataType (if any), and the second is True if the column header indicates the
            column is combinatorial
        """
        ...
        if assay_meta_type.postfix:
            singular_regex = _TYPE_NAME_REGEX % {
                ...
                "units": assay_meta_type.postfix,
            }
            if re.match(singular_regex, suffix, re.IGNORECASE):
                return assay_meta_type, False

        # otherwise, check whether the column header exactly matches the type name
        # (case-insensitive)
        elif suffix == upper_type_name:
            return assay_meta_type, False

        # if the column header didn't match the assay metadata type in its raw form, look for a
        # pluralized version of the metadata type name. Pluralization indicates the contents
        # should be treated as a comma-delimited list of combinatorial metadata values
        meta_regex = _PLURALIZED_REGEX % {
            "type_name": re.escape(assay_meta_type.type_name),
            "units": re.escape(assay_meta_type.postfix),
        }
        pluralized_match = re.match(meta_regex, suffix, re.IGNORECASE)

        if pluralized_match:
            logger.debug(
                f"Column header suffix {suffix} matched pluralized regex {meta_regex}"
            )
            return assay_meta_type, True
    ...
    def _parse_line_metadata_header(self, column_layout, std_cell_content, col_index):
        ...
        for std_type_name, meta_type in self.line_mtypes_by_name.items():
            ...
            if meta_type.postfix:
                singular_regex = _TYPE_NAME_REGEX % {
                    "type_name": std_type_name,
                    "units": meta_type.postfix,
                }
                result = (
                    meta_type
                    if re.match(singular_regex, std_cell_content, re.IGNORECASE)
                    else None
                )
            # otherwise, check whether the column header exactly matches the type name
            ...
            if pluralized_match:
                result = meta_type
                column_layout.register_line_meta_col(
                    col_index, result, is_combinatorial=True
                )
                logger.debug(
                    f'Column header "{std_cell_content}" matches line metadata type '
                    f'"{std_type_name}"'
                )
                return result
    ...
    def _parse_row(self, row, row_index) -> _ExperimentDescriptionFileRow | None:
        ...
        """
        row_inputs = _ExperimentDescriptionFileRow(
            self.column_layout, self.cache, row_index + 1, self.importer
        )
    ...
    def _parse_line_name(self, row, row_inputs, row_index):
        ...
        str_cell_content = self._get_str_cell_content(
            row, row_index, self.column_layout.line_name_col, convert_to_string=True
        )
        ...
        for cell in row:
            ...
            if cell_val is not None and str(cell_val).strip():
                ...
                logger.info(
                    f"Parse error: Cell {name_cell_num} was empty, but was expected "
        ...
        # track consecutive empty rows so we can just log one message for a range
        if self.first_empty_row is None:
            self.first_empty_row = row_index
        return False
    ...
    def _parse_description(self, row_inputs, row, row_index):
        ...
        str_cell_content = self._get_str_cell_content(row, row_index, desc_col)
        if str_cell_content:
    ...
    def _parse_control(self, row_inputs, row, row_index):
        ...
        str_cell_content = self._get_str_cell_content(row, row_index, ctrl_col)
    ...
    def _parse_replicates(self, row_inputs, row, row_index):
        ...
        if raw_val is None:
            ...
            self.importer.add_warning(
                IGNORED_INPUT_CATEGORY, ROWS_MISSING_REPLICATE_COUNT, row_index + 1
            )
            return
        ...
        try:
            ...
            if row_inputs.replicate_count == 0:
                ...
                self.importer.add_error(
                    BAD_GENERIC_INPUT_CATEGORY, ZERO_REPLICATES, cell
                )
        except ValueError:
            cell = cell_desc(raw_val, row_index, replicate_col)
            self.importer.add_error(
                BAD_GENERIC_INPUT_CATEGORY, INVALID_REPLICATE_COUNT, cell
            )
    ...
    def _parse_line_meta(self, row_inputs, row, row_index):
        ...
        for col_index, mtype_pk in col_to_meta_pk.items():
            ...
            if col_index in self.column_layout.combinatorial_col_indices:
                ...
                row_inputs.set_common_line_metadata(mtype_pk, str_cell_content)

    def _interpret_strains_content(self, content, row_index, row_inputs):
        strains_is_combo = (
            self.column_layout.strain_ids_col
            in self.column_layout.combinatorial_col_indices
        )
        ...
        tokens = filter(None, delim.split(content))
        ...
        for token_index, token in enumerate(tokens):
            ...
            cell_num = cell_coords(row_index, self.column_layout.strain_ids_col)
            ...
            if token_strain_grp_match:
                ...
                logger.debug(
                    f'Token "{token}" is a strain group. Match subgroup 1: "{grp}"'
                )
                strain_group = [
                    strain_id.strip()
                    for strain_id in token_strain_grp_match.group(1).split(
                        _STRAIN_GROUP_MEMBER_DELIM
                    )
                ]
    ...
    def parse_strains_cell(self, cols_list, row_index, row_inputs):
        ...
        cell_content = self._get_str_cell_content(
            cols_list, row_index, strain_ids_col, convert_to_string=True
        )

        if not cell_content:
            return

        individuals, groups = self._interpret_strains_content(
            cell_content, row_index, row_inputs
        )

        logger.debug(
            f"Done parsing strains for row {row_index + 1}. Individual strains: "
            f"{individuals}, combo_groups: {groups}"
        )

        # depending on whether the column header defines combinatorial input, either submit
        ...
        if col_combinatorial:
            if individuals:
                ...
                for part_id in individuals:
                    groups.append([part_id])

            if groups:
                row_inputs.combinatorial_line_metadata[strains_pk] = groups

        else:
            if individuals:
                row_inputs.common_line_metadata[strains_pk] = individuals
            elif groups:
                row_inputs.common_line_metadata[strains_pk] = groups
    ...
    def _check_part_id_pattern(self, part_id, input_location=""):
        ...
        if not part_number_match:
            ...
            logger.warning(
                f"Expected ICE part number(s) but {desc}, did not match the expected "
    ...
    def _parse_assay_metadata(self, row_inputs, row, row_index):
        ...
        for col_index, (protocol, assay_mtype) in assay_mtype_cols.items():
            ...
            error_key = (
                INCORRECT_TIME_FORMAT if is_time else UNPARSEABLE_COMBINATORIAL_VALUE
            )
            ...
            if col_index not in layout.combinatorial_col_indices:
                ...
                # best to catch errors here since time's currently stored in jsonbfield,
                # potentially as a string if incorrectly formatted.
                ...
                row_inputs.set_common_assay_metadata(
                    protocol.pk, assay_mtype.pk, cell_content
                )
                continue
    ...
    def _get_str_cell_content(self, row, row_index, col_index, convert_to_string=False):
        ...
        if isinstance(cell_content, str):
            ...
            if convert_to_string:
                ...
                self.importer.add_error(
                    INVALID_FILE_VALUE_CATEGORY, INVALID_CELL_TYPE, msg
                )
                return None

    # TODO: lots of refactoring to do here!
    def _parse_combinatorial_input(
        self,
        row_inputs,
        cell_content,
        row_index,
        ...
        for token in cell_content.split(","):
            ...
            try:
                ...
                if protocol:
                    row_inputs.add_combinatorial_assay_metadata(
                        protocol.pk, mtype_pk, parsed_val
                    )
                else:
...
class JsonInputParser(CombinatorialInputParser):
    ...
    def parse(
        ...
        parsed_json = self._validate_parsed_json(raw_json)
        if parsed_json is None:
            return
        elif not isinstance(parsed_json, Sequence):
        ...
        for value in parsed_json:
            ...
            common_line_metadata = _copy_to_numeric_keys(
                value.pop(COMMON_LINE_METADATA_SECTION, {})
            )
            ...
            ice_folder_to_filters = _copy_to_numeric_keys(
                value.pop("ice_folder_to_filters", {})
            )
            naming_strategy = self._get_naming_strategy(value)
            ...
            try:
                ...
                max_decimal_digits = self._check_time_size(protocol_to_assay_metadata)

                naming_strategy.combinatorial_input = combo_input
                combinatorial_inputs.append(combo_input)
            except RuntimeError as rte:
                logger.exception(
                    f"Unexpected parse error {rte}. Raw JSON is {raw_json}"
                )
                self.aggregator.add_error(
                    INTERNAL_EDD_ERROR_CATEGORY, PARSE_ERROR, str(rte)
                )
        ...
        for combo_input in combinatorial_inputs:
            combo_input.verify_pks(self.cache, self.aggregator)
    ...
    def _get_naming_strategy(self, value):
        ...
        if naming_elements:
            ...
            abbreviations = _process_abbrev_keys(
                naming_elements.get(ABBREVIATIONS_SECTION, {})
            )
            ...
            naming_strategy = _ExperimentDescNamingStrategy(
                ColumnLayout(self), self.cache
            )
            naming_strategy.base_line_name = base_name
        return naming_strategy

    def _check_time_size(self, protocol_to_assay_metadata):
        ...
        size = 0
        if assay_time_pk:
            for assay_metadata in protocol_to_assay_metadata.values():
