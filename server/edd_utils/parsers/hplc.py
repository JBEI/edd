...
CompoundRecord = namedtuple(
    "CompoundRecord", ["compound", "line", "assay", "timepoints"]
)
...
class HPLC_Parser(object):
    ...
    def __init__(self, input_stream):
        ...
        self.sample_name_regex = re.compile(
            r"(.*)_HPLC@([0-9]+(?:\.[0-9]*)?)(?:_([^@]+))?"
        )
    ...
    def parse_hplc(self):
        ...
        if firstline.startswith("Batch"):
            ...
            (header_block, table_header, table_divider) = self._get_table_header(
                header_block
            )
    ...
    def _decode_input_stream(self):
        ...
        try:
            ...
            raise HplcInputError(
                "unable to decode document using guessed encoding %s", encoding
            )
            ...
            for entry in sample:
                try:
                    ...
                    record = CompoundRecord(
                        entry.compound, line, assay, [[time or 0, value]]
                    )
    ...
    def _parse_96_well_format_block(
        ...
        for line_number, line in enumerate(self.decoded_stream):
            ...
            for index, header in enumerate(column_headers):
                if "Sample" in header:
                    begin_position = sum(section_widths[:index]) + len(
                        section_widths[:index]
                    )
