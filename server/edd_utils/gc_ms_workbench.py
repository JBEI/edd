...
def process_gc_ms_form_and_parse_file(form, file):
    try:
        ...
        try:
            ...
            return {"data_type": "xls", "headers": headers, "table": table}
...
def combine_processed_peaks_and_metadata(
    molecules, peak_table, annotation_headers, annotation_table
):
    """
    ...
    combined_col_names = ["Sample ID"] + [
        field for i, field in enumerate(annotation_headers) if i != i_sample
    ]
