...
def extract_non_blank_string_from_form(
    form, param_name, allow_list=False, return_none_if_missing=False
):
    if param_name not in form:
        if return_none_if_missing:
            return None
        raise KeyError(
            "The required parameter '%s' was not found in the submitted form."
            % param_name
        )
    ...
    if isinstance(str_value, list):
        if not allow_list:
            raise TypeError(
                "The parameter %s must be a scalar value, but a list was submitted."
                % param_name
            )
        return str_value
    if str_value == "":
...
def extract_integers_from_form(
    form, param_name, allow_list=False, return_none_if_missing=False
):
...
def _extract_numbers_from_form(
    form, param_name, param_type, allow_list=False, return_none_if_missing=False
):
    factory = {"integer": int, "decimal number": float}[param_type]
