"""
Defines remote tasks to be executed asynchronously by the Celery distributed task queue. To implement an asychronous
task to be remotely executed by Celery, define a carefully-defined function here and decorate it with @task_queue.task.
For information about Celery itself, see http://www.celeryproject.org/.
See celeryconfig.py for EDD's Celery configuration and defaults. Also see JBEI reminders list for Celery task
implementation. See celery_utils.py for common supporting utility methods.
"""
...
from edd.celeryconfig import CELERY_INITIAL_ICE_RETRY_DELAY, CELERY_WARN_AFTER_RETRY_NUM_FOR_ICE, CELERY_MAX_ICE_RETRIES
...
@shared_task(bind=True, default_retry_delay=2,
             max_retries=5)  # retry 5 times ~= 1 min total duration, with warning notification at ~6 s = #3
def test_repeated_retry_failure(self, fail_for_outdated_input=False, succeed_on_final_attempt=True):
    """
    A test task that simulates repeated failure. Helpful in testing error, warning, and resolution notifications, and
    serves as a basic example for future Celery task implementations.
    The task will repeatedly fail, sending a warning message to administrators prior to its eventual failure.
       :param fail_for_outdated_input: true to simulate task failure due to inputs being outdated (no retries will
       occur).
       :param succeed_on_final_attempt: true to simulate task success on the final attempt, resulting in a resolution
       notification
    :return:
    """
    ...
    # since it was submitted. Email administrators to warn them of a possible problem, then succeed.
        ...
        send_stale_input_warning(self, specific_cause, est_execution_time, use_exponential_backoff, celery_logger)
        return _STALE_OR_ERR_INPUT

    # assume all other Exceptions are the result of communication errors with EDD and/or ICE. Warn administrators
    # about the problem if this is the right time for a warning, then schedule the next execution attempt
        ...
        send_retry_warning_if_applicable(self, est_execution_time,
                                         ...
                                         skip_moot_warnings=False)  # compute a reasonable delay and schedule a retry
    ...
    # if we've published a warning message due to initial failure(s), publish a resolution message on success.
    # possible this will be delivered without any warning message if there wasn't time for anyone to address the
    # warning before the final execution attempt, but the resolution message may still be useful to have
...
def link_ice_entry_to_study(self, edd_user_email, strain_pk, study_pk, study_url, old_study_name=None, **kwargs):
    ...
    avoid (though not entirely prevent) race conditions, EDD updates the entire list of study links associated with that
    strain. As a future improvement, we could request that ICE expose atomic add/remove operations in its API, since
    that would cover the most common use cases, leaving only EDD strain creation/edits to update multiple sets of ICE
    links that could still be done as individual add or remove calls.config['db'].get('database', 'edd'),


    If the linking process fails, it will be retried until success, until the maximum number of retries is reached, or
    until the inputs are stale due to concurrent user edits via the EDD user interface.
    TODO: Note that throughput would be faster if we passed a list of (edd_study_id, ice_strain_id) tuples to this
    ...
    try:
        # Verify that the EDD study->line->strain relationship that motivated this ICE push is still valid. Since
        # execution of this task may be delayed due to high load, or since the task may have been retried
        # multiple times over the course of a significant time period, it's always possible
        # that EDD data has been changed since the ICE communication task was pushed to Celery. Our initial settings
        # assume that the primary reason for failure of this task is a network partition or short downtime for ICE or
        # EDD, and we automatically retry the inter-system communication to reduce the system administration burden for
        # EDD. However, this creates a situation where ICE may be unreachable for minutes, hours, or days (depending on
        ...
        # as a workaround for SYNBIO-1207, prefer the ICE id extracted from the URL, which is much more likely to
    ...
    # if Celery is about to forcibly terminate the worker, perform cleanup and then fail the task by re-raising
    # the Exception
    except SoftTimeLimitExceeded as stl:
        ...
        # if conditions are met, send a warning email to administrators so they can take action
        # before the task fails and requires more manual effort to correct
        send_retry_warning_if_applicable(self, est_execution_time, CELERY_WARN_AFTER_RETRY_NUM_FOR_ICE,
                                         celery_logger)
